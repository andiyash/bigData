...
public class LongRidesExercise {
    ...
    public static class AlertFunction extends KeyedProcessFunction<Long, TaxiRide, Long> {
        ...
        public void open(Configuration config) {
            // Инициализация состояния ValueState в методе open
            ValueStateDescriptor<TaxiRide> rideStateDescriptor =
                    new ValueStateDescriptor<>("событие поездки", TaxiRide.class);
        ...
        public void processElement(TaxiRide ride, Context context, Collector<Long> out) throws Exception {
            ...
            if (firstRideEvent == null) {
                // Запоминаем первое событие поездки
                rideState.update(ride);

                if (ride.isStart) {
                    // Устанавливаем таймер для проверки слишком долгих поездок
                    context.timerService().registerEventTimeTimer(getTimerTime(ride));
                }
            } else {
                if (ride.isStart) {
                    // Проверяем, слишком ли долгая поездка
                    if (rideTooLong(ride, firstRideEvent)) {
                        out.collect(ride.rideId);
                    }
                } else {
                    // Удаляем таймер для первой поездки, так как встречено событие END
                    context.timerService().deleteEventTimeTimer(getTimerTime(firstRideEvent));

                    // Проверяем, не стала ли поездка слишком долгой
                    if (rideTooLong(firstRideEvent, ride)) {
                        out.collect(ride.rideId);
                    }
                }

                // Очищаем состояние, так как оба события были учтены
                rideState.clear();
            }
        }
        ...
        public void onTimer(long timestamp, OnTimerContext context, Collector<Long> out) throws Exception {
            // Таймер срабатывает только в случае слишком долгой поездки
            out.collect(rideState.value().rideId);

            // Очистка состояния предотвращает дублирование предупреждений
            rideState.clear();
        }
        ...
        private long getTimerTime(TaxiRide ride) throws RuntimeException {
            if (ride.isStart) {
                ...
                throw new RuntimeException("Невозможно получить время начала из события END.");
            }
        }
    }
}
